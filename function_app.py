...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        selected_col = ['Kategori P', 'Luas Bangu', 'Luas Tanah', 'Jenis Sert',  'Latitude', 'Longitude', 'Dekat Exit Tol']
        # 'Rasio Luas Bangunan',
        ...
        # def predict_for_year(year):
        #     # Ambil kolom tambahan (misal 'Permukiman Perkotaan_2030')
        #     year_cols = [col for col in df_combined.columns if str(year) in col]

        #     # Mapping nama kolom ke bentuk yang dikenali preprocessor
        #     rename_map = {col: col.replace(f'_{year}', '').strip() for col in year_cols}

        #     # Ambil semua fitur yang dibutuhkan
        #     features = selected_col + year_cols
        #     X = df_combined[features].copy()

        #     # Rename hanya kolom tahun → agar match ke preprocessor
        #     X.rename(columns=rename_map, inplace=True)

        #     # Pastikan urutan dan jumlah kolom cocok
        #     missing = set(preprocessor.feature_names_in_) - set(X.columns)
        #     if missing:
        #      raise ValueError(f"Missing columns for year {year}: {missing}")
